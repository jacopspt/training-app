...
- **📊 Volume per gruppo** (in Schede) — tracker in presa diretta delle serie assegnate a ogni gruppo muscolare mentre componi il programma: barre ordinate per volume, totale e numero di esercizi. Di base somma **tutte le sedute** (Scheda A + B + C…), che è il volume settimanale vero; con **Solo questa** si isola la seduta aperta. Segue i tasti *Vista*: con una settimana selezionata conta quella settimana, con *Tutte* mostra la media a settimana. Si apre e chiude con **▾ Nascondi / ▸ Mostra** e la scelta viene ricordata. Gli **EMOM valgono i loro minuti** (EMOM 5′ × 3 rep = 5 serie per il gruppo coinvolto); senza minuti indicati valgono 1. Le righe senza nome non vengono conteggiate e sono segnalate
